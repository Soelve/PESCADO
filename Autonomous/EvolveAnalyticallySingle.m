...
DensMatEnergyAfter = zeros(N, N, lmax+1);
for L = 0:lmax
  % Write L to screen
  disp(['L=',num2str(L)])
  for nL = 1:N
    ...
    if imag(En) < MaxImagE
      ...
      Cn = Cmat(nL, L+1);
      for mL = 1:N
        Em = BigCmplxEmat(mL, L+1);
        Cm = Cmat(mL, L+1);
        Phi_m = BigUmat(:,mL,L+1);
        DensMatEnergyAfter(:,:, L+1) = ...
        DensMatEnergyAfter(:,:,L+1) + ...
...
% Multiply -i
DensMatEnergyAfter = -1i*DensMatEnergyAfter;